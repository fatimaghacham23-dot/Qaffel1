...
const base = () => ({ onboardingEvidence: createOnboardingEvidenceFixture(), pendingProofCount: 0, rejectedProofCount: 0, pendingInvitationCount: 0, assignmentCount: 0, invoices: [], now });
...
describe("derived notifications", () => {
  ...
  it("filters categories and keeps only the roles that have the required permission", () => {
    const items = buildDerivedNotifications({ ...base(), pendingProofCount: 1, onboardingEvidence: createOnboardingEvidenceFixture({ hasActivePaymentMethod: false }) });
  ...
  it("removes onboarding items as real setup becomes complete", () => {
    const incomplete = buildDerivedNotifications({ ...base(), onboardingEvidence: createOnboardingEvidenceFixture({ hasCompleteBusinessIdentity: false, hasActivePaymentMethod: false, hasClient: false, hasInvoice: false, hasSharedPaymentRequest: false, missingBusinessIdentityFields: ["business_name", "support_contact"] }) });
