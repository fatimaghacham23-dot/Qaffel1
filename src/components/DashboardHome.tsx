...
const activityIcon: Record<RecentActivityItem["type"], typeof FileText> = { invoice_created: FileText, client_created: Users, invoice_shared: FileText, proof_uploaded: ShieldCheck, proof_approved: Check, proof_rejected: CircleAlert, payment_received: ReceiptText, receipt_issued: ReceiptText };
  ...
  return (
    <main className="mx-auto max-w-[1480px]">
      ...
      <div className={`mt-7 grid gap-7 ${showMetrics ? "xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]" : "max-w-4xl"}`}>
        ...
        {props.capabilities.showActivity ? <section aria-labelledby="activity-heading" className="rounded-3xl bg-white p-5 shadow-card sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="q-section-label">Latest</p><h2 id="activity-heading" className="mt-1 text-xl font-semibold text-ink">Recent activity</h2></div><Link href="/reports" className="text-xs font-semibold text-cedar">View all activity</Link></div>{props.recentActivity.length ? <div className="mt-5 space-y-1">{props.recentActivity.map((item) => { const Icon = activityIcon[item.type]; return <Link href={item.href || "/dashboard"} key={item.id} className="flex items-start gap-3 rounded-xl px-2 py-3 transition hover:bg-slate-50"><span className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-slate-100 text-cedar"><Icon className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-sm font-medium text-ink">{item.title}</span><span className="mt-1 block text-xs text-slate-500">{shortDate(item.occurredAt)}</span>{item.description ? <span className="mt-1 block text-xs text-slate-500">{item.description}</span> : null}</span></Link>})}</div> : <div className="mt-6 rounded-2xl bg-slate-50 p-6 text-center"><Clock3 className="mx-auto h-5 w-5 text-slate-400" /><p className="mt-3 text-sm font-semibold text-ink">No recent activity yet</p><p className="mt-1 text-xs leading-5 text-slate-500">Meaningful invoice and payment updates will appear here.</p></div>}</section> : null}
      </div>
