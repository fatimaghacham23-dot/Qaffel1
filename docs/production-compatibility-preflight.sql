...
WITH target_tables(schema_name, table_name) AS (
  ...
  UNION ALL
  SELECT 'BACKFILL', 'clients.workspace_id null count', '0', count(*)::text, CASE WHEN count(*)=0 THEN 'READY' ELSE 'BLOCKED' END, 'HIGH', 'Workspace RLS requires backfill; absent optional column is counted as null' FROM public.clients c WHERE to_jsonb(c)->>'workspace_id' IS NULL
  UNION ALL SELECT 'BACKFILL', 'invoices.workspace_id null count', '0', count(*)::text, CASE WHEN count(*)=0 THEN 'READY' ELSE 'BLOCKED' END, 'HIGH', 'Workspace RLS requires backfill; absent optional column is counted as null' FROM public.invoices i WHERE to_jsonb(i)->>'workspace_id' IS NULL
  ...
  UNION ALL SELECT 'RPC_DEPENDENCY_STATUS', 'payment_proofs accepted/pending/rejected/voided', 'status constraint supports values', COALESCE((SELECT string_agg(pg_get_constraintdef(c.oid), ' | ') FROM pg_constraint c JOIN pg_class t ON t.oid=c.conrelid JOIN pg_namespace n ON n.oid=t.relnamespace WHERE n.nspname::text='public' AND t.relname::text='payment_proofs' AND c.contype::text='c'),'table or check constraint absent'), 'REVIEW', 'HIGH', 'Review current status constraint before atomic functions'
